# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

fast_apply_nb_na <- function(X, dim) {
    .Call('imp4p_fast_apply_nb_na', PACKAGE = 'imp4p', X, dim)
}

fast_apply_nb_not_na <- function(X, dim) {
    .Call('imp4p_fast_apply_nb_not_na', PACKAGE = 'imp4p', X, dim)
}

fast_apply_sd_na_rm_T <- function(X, dim) {
    .Call('imp4p_fast_apply_sd_na_rm_T', PACKAGE = 'imp4p', X, dim)
}

fast_apply_sum_na_rm_T <- function(X, dim) {
    .Call('imp4p_fast_apply_sum_na_rm_T', PACKAGE = 'imp4p', X, dim)
}

fast_sim <- function(prot, mat) {
    .Call('imp4p_fast_sim', PACKAGE = 'imp4p', prot, mat)
}

